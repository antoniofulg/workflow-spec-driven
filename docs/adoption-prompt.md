...
```
Adopt selected layers of the agent OS from /path/to/antoniofulg-workflow-spec-driven-0.10.0.tgz into /path/to/target-project.
...
`extras`, or `full`). Run `npm exec --yes --package /path/to/antoniofulg-workflow-spec-driven-0.10.0.tgz -- my-workflow plan /path/to/target-project --layers <selected-layers> --json` and review its actions. Report the managed paths and every target path that could be replaced. Preserve
...
then run `npm exec --yes --package /path/to/antoniofulg-workflow-spec-driven-0.10.0.tgz -- my-workflow resolve /path/to/target-project --layers <selected-layers> --replace <reviewed-file> [--replace <reviewed-file> ...]`, usually with
...
an update. Run `npm exec --yes --package /path/to/antoniofulg-workflow-spec-driven-0.10.0.tgz -- my-workflow apply /path/to/target-project --layers <selected-layers>` only after the review. Use the same `<selected-layers>` value in plan and apply; omitting it selects `full`. A normal apply promotes pristine provider templates, refreshes managed blocks and runtime packets, and reconciles only hash-proven retired workflow files.
