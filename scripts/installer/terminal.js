...
import { buildPlan, gitProof, LAYERS, resolveModules, safePath } from './engine.js';
...
export async function runInstallWizard({ targetRoot = process.cwd(), sourceRoot, input = async () => null, write = console.log, width = Number(process.stdout.columns) || 80, color = !process.env.NO_COLOR, planner = buildPlan, transaction = applyTransaction, requireGit = false } = {}) {
  ...
  let built = addPacketActions(planner({ sourceRoot, targetRoot: root, selectedModules: selected }), packageRoot, root); if (built.plan.message) { print(built.plan.message); print('No backup required.'); return { plan: built.plan, code: 0 }; }
  ...
  while (built.plan.unresolved?.length) {
    ...
    built = addPacketActions(planner({ sourceRoot, targetRoot: root, selectedModules: modulesAfter }), packageRoot, root); for (const item of built.plan.actions) if (replacements.includes(item.path)) item.kind = 'replace'; built.plan.unresolved = built.plan.unresolved.filter((item) => !replacements.includes(item));
  }
