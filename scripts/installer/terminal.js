...
export async function runInstallWizard({ targetRoot = process.cwd(), sourceRoot, input = async () => null, write = console.log, width = Number(process.stdout.columns) || 80, color = !process.env.NO_COLOR, planner = buildPlan, transaction = applyTransaction } = {}) {
  ...
  const prepared = { targetRoot: root, plan: built.plan, staged: built.staged }; for (const action of built.plan.actions.filter((item) => item.kind === 'replace')) prepared.staged[action.path] = (await import('node:fs')).readFileSync(path.join(packageRoot, action.path));
