...
export async function runInstallWizard({ targetRoot = process.cwd(), sourceRoot, input = async () => null, write = console.log, width = Number(process.stdout.columns) || 80, color = !process.env.NO_COLOR, planner = buildPlan, transaction = applyTransaction, requireGit = false } = {}) {
  ...
  let built = addPacketActions(planner({ sourceRoot, targetRoot: root, selectedModules: selected }), packageRoot, root); if (built.plan.message) { print(built.plan.message); print('No backup required.'); print('\nExternal security skills remain separately authorized.'); print(securityInstallCommand(packageRoot, root)); return { plan: built.plan, code: 0 }; }
