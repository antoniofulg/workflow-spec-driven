...
export function renderPlan(plan, width = 80) {
  ...
  for (const action of plan.actions || []) { const modules = (action.modules || []).join(', '); const prefix = width >= 100 ? `  ${(labels[action.kind] || action.kind).padEnd(10)} ${modules.padEnd(10)} ` : `  ${(labels[action.kind] || action.kind).padEnd(9)} `; const suffix = action.reason ? `${width >= 100 ? '' : 'module: '}${modules}${width >= 100 ? ` (${action.reason})` : `; ${action.reason}`}` : `module: ${modules}`; const row = width >= 100 && action.reason ? `${prefix}${action.path} (${action.reason})` : `${prefix}${action.path}`; lines.push(...line(row, width), ...(width < 100 ? line(`            ${suffix}`, width) : [])); }
