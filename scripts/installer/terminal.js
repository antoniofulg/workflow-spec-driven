...
import { buildPlan, LAYERS, resolveModules } from './engine.js';
import { applyTransaction, hasInterruptedTransaction, restoreInterrupted } from './transaction.js';
import { prepareKnowledge, knowledgeTransfers } from './knowledge.js';
...
export function renderPlan(plan, width = 80) {
  ...
  for (const action of plan.actions || []) { const modules = (action.modules || []).join(', '); const suffix = action.reason ? `module: ${modules}; ${action.reason}` : `module: ${modules}`; lines.push(`  ${(labels[action.kind] || action.kind).padEnd(9)} ${action.path}`, ...line(`            ${suffix}`, Math.max(20, width))); }
...
export async function runInstallWizard({ targetRoot = process.cwd(), sourceRoot, input = async () => null, write = console.log, width = Number(process.stdout.columns) || 80, color = !process.env.NO_COLOR, planner = buildPlan, transaction = applyTransaction } = {}) {
  ...
  const prepared = { targetRoot: root, plan: built.plan, staged: built.staged }; for (const action of built.plan.actions.filter((item) => item.kind === 'replace')) prepared.staged[action.path] = (await import('node:fs')).readFileSync(path.join(sourceRoot || path.resolve(path.dirname(new URL(import.meta.url).pathname), '../..'), action.path));
  let result; try { result = transaction(prepared); } catch (error) { print(`[ERROR] ${error.message}`); return { code: 1, error }; } const items = knowledgeTransfers(built.plan, result); if (result.backup && items.length) prepareKnowledge(built.plan, { ...prepared, backup: result.backup }); print('\nInstallation complete.'); print(`Modules: ${built.plan.selectedModules.join(', ')}`); print(`Backup: ${result.backup || 'No backup required.'}`); if (items.length) { print('\nKnowledge transfer required:'); items.forEach((item) => print(`  Source: ${item.source}\n  Destination: ${item.destination}\n  Reason: ${item.reason}\n  Status: ${item.status}`)); } return { plan: built.plan, result, knowledge: items, code: 0 };
}
