...
Freeze the obligations. Free the plan. Prove it with someone who did not build it. Derived from tlc-spec-driven 3.3.0 (Felipe Rodrigues), wtk-plan, and wtk-implement.
...
`light`. Same three levels as `wtk-implement`, gated the same way, so moving between the two
...
The profile is a pin, not a preference, and unlike `wtk-implement` that is enforced rather than
...
User says: "wtk-lean — plan the lockfile v2 migration"
Actions:
...
1. The orchestrator — not the builder — dispatches one fresh Verifier over `<feature base>..HEAD` with every check
