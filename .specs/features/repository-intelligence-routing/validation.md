...
| R14 | All Done-when boxes checked in `tasks.md:617-622` | Independently verified: real code-only setup/status passes and malformed `--backend code-only` is killed. |
...
| RIR-01.4 | Every named Graft failure reports one degraded reason, targeted fallback, and dedicated non-zero exit. | `tools/test_repository_intelligence.py:123-148,174-207,490-538`; literal exit assertions at `:127` and `:518`. | PASS |
...
| RIR-02.3 | Design retains bounded domains, relationships, paths, and risks. | `tools/test_repository_intelligence.py:540-544`; `tools/test_phase_skills.py:155-164`. | PASS |
...
| RIR-02.5 | Missing backend, timeout, budget, partial, failure, and insufficient Graphify results degrade explicitly; code-only setup is a partial local mode without a backend flag. | `tools/test_repository_intelligence.py:89-93,209-215,315-324,412-488,540-544`; exact partial/code-only assertions at `:470-482`; `tools/test_deep_review_token_metrics.py:958-988,1051-1089`. | PASS |
...
| RIR-04.1 | State binds checkout, exact version, backend/scope, manifest, and tree; code-only persists backend `code-only` with partial status. | `tools/test_repository_intelligence.py:95-115,220-230,447-488`; exact persisted status assertions at `:475-482`. | PASS |
| RIR-04.2 | Indexed source/config/docs changes refresh or invalidate, including tracked directory-symlink target bytes; Graphify refresh uses `update <source-path>` and forced rebuild uses `update <source-path> --force`. | Refresh/fingerprint assertions at `tools/test_repository_intelligence.py:232-269,546-561`; R13 logs rejected calls before validation and asserts the first update and forced recovery argv separately at `:412-445`. | PASS |
| RIR-04.3 | Mutations serialize; reads use completed state. | `tools/test_repository_intelligence.py:289-312,598-611`. | PASS |
...
| RIR-04.7 | Interrupted publication preserves matching complete state or unavailable state. | `tools/test_repository_intelligence.py:271-287,390-410,563-579`. | PASS |
| RIR-05.1 | Benchmark record contains every required metric and terminal evidence field. | `tools/test_repository_intelligence.py:615-624,663-669,686-715,730-735`. | PASS |
| RIR-05.2 | Compared records match all declared controls. | `tools/test_repository_intelligence.py:626-642,737-744`. | PASS |
| RIR-05.3 | First phase compares baseline to Graft. | `tools/test_repository_intelligence.py:671-676`. | PASS |
| RIR-05.4 | Routed phase compares Graft to routed within category. | `tools/test_repository_intelligence.py:644-654`. | PASS |
| RIR-05.5 | Directional report requires 10–20 distinct terminal tasks. | `tools/test_repository_intelligence.py:644-660,678-684`. | PASS |
| RIR-05.6 | Removal requires explicit decision over all named surfaces. | `tools/test_repository_intelligence.py:717-728`. | PASS |
...
| SEC-002 | Generated state is ignored; only promoted reports may be committed. | `.gitignore:19-22`; `.ignore:1-7`; `tools/test_repository_intelligence.py:581-592`; `tests/installer/acceptance.test.js:43`. | PASS |
...
- **Adapter scoped**: `python3 tools/test_repository_intelligence.py` — 64 passed, 0 failed, 0 skipped; exit 0.
...
- **Full**: `bun run test:all` — 126 Bun + 195 Node + 538 Python = 859 passed, 0 failed, 0 skipped; exit 0.
- **Real Graphify 0.9.14 code-only setup/status**: `python3 .agents/skills/workflow-spec-driven/scripts/repository_intelligence.py graphify-setup --root "$PWD" --backend code-only --mode code-only` then `status --root "$PWD" --json` — both exit 0; setup indexed 833 files, returned `status: partial`, and persisted Graphify `status: partial`, `backend: code-only`.
...
- **Before/after count**: 783 before, 859 after, delta +76. Derived from checkpoint baselines plus R14's one canonical adapter regression. No test deletion or skip was found.
- **Integrity verdict**: PASS. No test deletion, weakening, skip, or hollow assertion was found.

## R14 Independent Verification

- Production at `.agents/skills/workflow-spec-driven/scripts/repository_intelligence.py:431-463` builds `extract <source> --code-only --out <checkout>` for code-only and `extract <source> --mode <mode> --backend <backend> --out <checkout>` for semantic extraction.
- Public adapter regression at `tools/test_repository_intelligence.py:447-488` drives the CLI through a real subprocess, requires exact code-only and semantic argv, and asserts setup/status `partial` with backend `code-only`.
- Fresh adapter gate: `python3 tools/test_repository_intelligence.py` — 64 passed, 0 failed, 0 skipped; exit 0.
- Fresh installer parity: `node --test tests/installer/acceptance.test.js` — 42 passed, 0 failed, 0 skipped; exit 0.
- Fresh Review gate: `python3 tools/test_deep_review_contract.py && python3 tools/test_deep_review_token_metrics.py` — 47 + 32 = 79 passed, 0 failed, 0 skipped; exit 0.
- Fresh full gate: `bun run test:all` — 126 Bun + 195 Node + 538 Python = 859 passed, 0 failed, 0 skipped; exit 0.
- Real Graphify 0.9.14 code-only setup/status exits 0, indexes 833 files, and records `partial` / `code-only` on the active checkout.
- Fresh malformed-backend mutant adds `--backend code-only`; canonical adapter kills it with 64 tests run, 1 failure at `tools/test_repository_intelligence.py:470` and exit 1. Real checkout remains clean after disposable worktree removal.
...
Fresh R14 disposable sensor worktree at `46c33dec`; removed after use. Real checkout `git status --porcelain=v1` matched its clean baseline after cleanup. The malformed-backend mutant failed because code-only extraction cannot accept `--backend code-only`. Earlier M1–M8 remain closed evidence from prior verification.
...
| M9 (R14) | Add malformed `--backend code-only` to local code-only extraction. | `python3 tools/test_repository_intelligence.py` in disposable sensor worktree at `46c33dec` | **KILLED by fresh Technical Verifier**: 64 tests run, 1 failed at `tools/test_repository_intelligence.py:470`; setup exited 3 instead of 0. |

**Sensor depth**: expanded lightweight R12–R14 argv sensor. **Result**: 5/5 recorded argv mutants killed; PASS.
...
| Per-layer coverage | PASS: incremental update argv and code-only/semantic extraction argv are independently observable through public adapter subprocesses. |
| Every in-scope test maps to a requirement | PASS; no unclaimed added test found. |
| No hollow/wrong-layer tests | PASS under `docs/guidelines/TEST-CONTRACT.md`; R13 distinguishes malformed incremental argv from successful fallback, and R14 distinguishes the forbidden code-only backend flag through the public subprocess boundary. |
...
- R14 produced no finding fingerprint: its malformed-backend mutant was killed on the first fresh verification.
...
- Existing grounded candidate `L-112` remains recorded from R12's surviving mutant. This clean R14 PASS adds no lesson, per `wverify` clean-run rules.

## Summary

**Overall**: PASS. All 35 acceptance/security criteria have exact behavioral evidence. Adapter passes 64/64, installer parity 42/42, Review 79/79, full gate 859/859, real Graphify 0.9.14 code-only setup/status passes with `partial` / `code-only`, all 5 recorded argv mutants are killed, and no fingerprint remains open.
