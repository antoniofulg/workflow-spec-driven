...
| IT-013 | Reconciles retired managed paths safely | Prior manifest contains a pristine managed retired workflow file, an edited managed retired workflow file, an absent managed retired workflow file, an absent unlisted managed file, and a consumer-owned retired file across isolated cases | Pristine workflow file is previewed/removed during staged publication; edited and present unlisted files conflict with zero writes even with matching hashes; absent managed and unlisted files are accepted and stop tracking; consumer file remains byte-identical; no installed layer is removed. |
