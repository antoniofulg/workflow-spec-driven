...
subagent prompt, fix, and review, until the human explicitly says `stop ponytail` or `normal mode`. After each coherent edit batch, run the project’s existing formatter only on changed files before validation. Keep successful formatter output silent; show concise diagnostics only on failure.
