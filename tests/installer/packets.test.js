...
test('IT-011 generated provider packet catalog is complete', () => { const packets = stageAgentPackets(root, root); assert.equal(Object.keys(packets).filter((key) => key.includes('/agents/')).length, 18); assert.ok(packets['.claude/agents/deep-reviewer.md']); assert.ok(packets['.codex/agents/deep-reviewer.toml']); assert.ok(packets['.cursor/agents/deep-reviewer.md']); });
...
test('T2 parser rejects malformed TOML', () => { const dir = temp(); fs.writeFileSync(path.join(dir, '.wtk.toml'), 'version = ['); assert.throws(() => readWorkflowConfig(dir), /invalid/); });
...
test('T2 Claude rendering changes model and effort only', () => { const original = fs.readFileSync(path.join(root, '.agents/skills/wtk-config/assets/agents/claude/planner.md')); const rendered = renderAgentPacket('claude', original, { model: 'new-model', effort: 'low' }); assert.equal(packetSetting('claude', rendered).model, 'new-model'); assert.match(rendered.toString(), /effort: low/); });
test('T2 Cursor rendering changes combined metadata', () => { const original = fs.readFileSync(path.join(root, '.agents/skills/wtk-config/assets/agents/cursor/planner.md')); const rendered = renderAgentPacket('cursor', original, { model: 'new-model', effort: 'low' }); assert.deepEqual(packetSetting('cursor', rendered), { model: 'new-model', effort: 'low' }); });
test('T2 Codex rendering escapes quoted values', () => { const original = fs.readFileSync(path.join(root, '.agents/skills/wtk-config/assets/agents/codex/planner.toml')); const rendered = renderAgentPacket('codex', original, { model: 'new-model', effort: 'low' }); assert.deepEqual(packetSetting('codex', rendered), { model: 'new-model', effort: 'low' }); });
...
test('T2 staging does not write target runtime', () => { const dir = temp(); fs.cpSync(path.join(root, '.agents'), path.join(dir, '.agents'), { recursive: true }); fs.copyFileSync(path.join(root, '.wtk.toml.example'), path.join(dir, '.wtk.toml.example')); const before = fs.readdirSync(dir); stageAgentPackets(dir, dir); assert.deepEqual(fs.readdirSync(dir), before); });
